...
const downHandler = (req, res) => {
  ...
  connectDB()
    .then(() => {
      if (req.method === "POST") {
        ...
        User.exists({})
              ...
              User.findOne({})
                .then((user) => {
                  ...
                  if (isToday(new Date(user.lastUpdated))) {
                    ...
                    downLogger.info({
                      data: { msg: "User already updated today" },
                    });
                    res.status(200).json({
                      result: false,
          ...
          .catch((errFOU) => {
            downLogger.info({ data: { path: "/api/down", value: 10 } });
            res.send({
              message: "Error finding user",
              errs: ["Error finding user"],
              result: false,
    ...
    .catch((err) => {
      downLogger.error({ data: { err: err } });
      res.send({
        message: "Error connecting to database",
        errs: ["Error connecting to database"],
        result: false,
      });
    });
};
